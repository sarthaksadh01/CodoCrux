...
{
    $link = mysqli_connect("localhost","root","","avik");
  ...
  if(isset($_SESSION['email'])){
   ...
   $link = connect_to_database();
    ...
    if($qno==1)
    {
      ...
      $set = get_set();
      $query2 = "SELECT * FROM `questions` WHERE id = '$qno' AND s ='$set'";
    }
    ...
    if($result2) {
      ...
      if($qno==1){
         ...
         if($row2['one']==$ans)
         {
             $query3 = "UPDATE `users` SET prob = prob+1,s = 'one'  WHERE email ='$email'";
         ...
         else if($row2['two']==$ans){

           $query3 = "UPDATE `users` SET prob = prob+1,s = 'two'  WHERE email ='$email'";
           $result3 = mysqli_query($link,$query3);
           if($result3){
             echo "Correct Answer!";
           }
         ...
         else if($row2['three']==$ans){

           $query3 = "UPDATE `users` SET prob = prob+1,s = 'three'  WHERE email ='$email'";
           $result3 = mysqli_query($link,$query3);
           if($result3){
             echo "Correct Answer!";
           }
         ...
         else if($row2['four']==$ans){

           $query3 = "UPDATE `users` SET prob = prob+1,s = 'four'  WHERE email ='$email'";
           $result3 = mysqli_query($link,$query3);
           if($result3){
             echo "Correct Answer!";
           }
